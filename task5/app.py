...
def query():
    if request.method == 'GET':
        ...
        if has_result == 1 :
            ...
            # return render_template('index.html',q=q,results=results,error=error,search_info=search_info,has_previous=has_previous,current_start_index=current_start_index,page_index=page_index)
        else :
            search_info =  'About ' + json_data['searchInformation']['formattedTotalResults'] + ' results (' + json_data['searchInformation']['formattedSearchTime'] + ' seconds)'
            return render_template('index.html',q=q,error=error,engine_name=engine_name,search_info=search_info)
            # return render_template('index.html',q=q,error=error,search_info=search_info)
